#!/usr/bin/env python3
"""Apply a trained upper-body plus translation correction model to prediction PKLs."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-subdir", default="fused_params_upperbody_transl_corrected")
    ...
    parser.add_argument("--max-delta-degrees", type=float, default=60.0)
    parser.add_argument("--max-transl-delta-m", type=float, default=1.0)
...
def clamp_vector(vector: np.ndarray, max_norm: float) -> np.ndarray:
    output = np.asarray(vector, dtype=np.float32).reshape(3).copy()
    if max_norm > 0:
        norm = float(np.linalg.norm(output))
        if norm > max_norm:
            output = output * (float(max_norm) / norm)
    return output


def normalize_indices(values) -> list[int]:
    if values is None:
        raise ValueError("Checkpoint is missing target_body_indices")
...
def apply_delta(
    ...
    expected = len(body_indices) * 3 + 3
    if delta.shape[0] != expected:
        raise ValueError(f"Expected {expected} correction values, got {delta.shape[0]}")

    body = np.asarray(output["body_pose"], dtype=np.float32).reshape(21, 3).copy()
    rot_delta = delta[:-3].reshape(len(body_indices), 3)
    for index, delta_rotvec in zip(body_indices, rot_delta):
        body[index] = compose_rotvec(body[index], delta_rotvec, max_degrees, rot_scale)
    output["body_pose"] = body.reshape(-1)

    transl_delta = clamp_vector(delta[-3:] * float(transl_scale), max_transl_delta_m)
    output["transl"] = (np.asarray(output["transl"], dtype=np.float32).reshape(3) + transl_delta).astype(np.float32)
...
def main() -> None:
    ...
    for sequence in sequences:
        ...
        for row, delta in zip(rows, deltas):
            corrected = apply_delta(
                row["person"],
                delta,
                body_indices,
                args.max_delta_degrees,
                args.rot_scale,
                args.transl_scale,
                args.max_transl_delta_m,
            )
            ...
            rows_written.append(
                {
                    ...
                    "rot_delta_l2": float(np.linalg.norm(delta[:-3])),
                    "transl_delta_l2": float(np.linalg.norm(delta[-3:])),
                }
            )

    report_path = args.report or output_root / "evaluation" / f"apply_{args.output_subdir}" / "apply_report.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    csv_path = report_path.with_name("applied_upperbody_transl_corrections.csv")
    if rows_written:
    ...
    report = {
        ...
        "target_kind": checkpoint.get("target_kind", "upperbody_transl"),
        "target_body_indices": body_indices,
        "target_smplx_joints": checkpoint.get("target_smplx_joints", []),
        "rot_scale": args.rot_scale,
        "transl_scale": args.transl_scale,
        "max_delta_degrees": args.max_delta_degrees,
        "max_transl_delta_m": args.max_transl_delta_m,
