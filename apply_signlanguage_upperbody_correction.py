#!/usr/bin/env python3
"""Apply a trained SignLanguage upper-body correction model to prediction PKLs."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-subdir", default="fused_params_upperbody_corrected")
...
def apply_upperbody_delta(
    ...
    delta = np.asarray(delta, dtype=np.float32).reshape(len(body_indices), 3)
    for index, delta_rotvec in zip(body_indices, delta):
        body[index] = compose_rotvec(body[index], delta_rotvec, max_degrees, scale)
...
def main() -> None:
    ...
    for sequence in sequences:
        ...
        for row, delta in zip(rows, deltas):
            corrected = apply_upperbody_delta(row["person"], delta, body_indices, args.max_delta_degrees, args.scale)
    ...
    csv_path = report_path.with_name("applied_upperbody_corrections.csv")
    if rows_written:
