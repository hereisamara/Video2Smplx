#!/usr/bin/env python3
"""Train a visibility-weighted MLP for SignLanguage upper-body rotation correction."""
...
def eval_split(torch, model, loader, device, weight) -> dict[str, float]:
    ...
    if not losses:
        return {"loss": float("nan"), "unweighted_loss": float("nan"), "rot_l2_deg": float("nan")}
    return {
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"rot={row['val_rot_l2_deg']:.2f}deg"
        )
            ...
            torch.save(
                {
                    ...
                    "target_mode": str(scalar_from_npz(data, "target_mode", "upper_body")),
