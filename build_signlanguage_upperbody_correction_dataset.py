#!/usr/bin/env python3
"""Build a GT-supervised upper-body correction dataset for SignLanguage predictions."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", choices=tuple(TARGET_JOINTS), default="upper_body")
...
def target_weights(target_mode: str) -> np.ndarray:
    ...
    return np.repeat(weights, 3).astype(np.float32)


def upperbody_target(person: dict, gt_param: dict, target_mode: str) -> np.ndarray:
    ...
    values = [rotation_delta(pred_body[index], gt_body[index]) for index in target_body_indices(target_mode)]
    ...
    for sequence in sequences:
        ...
        for item in sequence_items.get(sequence, []):
            ...
            try:
                ...
                continue
            target = upperbody_target(person, gt_records[item["ground_truth_id"]]["smplx_param"], args.target)
